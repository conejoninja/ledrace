...
func main() {
	...
	// Initialize ESP, connect to MQTT server and hope it worked
	initESPAndMQTT()

	players := make([]info.Player, config.PLAYERS)

	// MQTT controllers
	players[0].Configure(input.NewMQTTController(&cl, "player1"), color.RGBA{255, 0, 0, 255})
	players[1].Configure(input.NewMQTTController(&cl, "player2"), color.RGBA{0, 255, 0, 255})

	// Normal button controllers
